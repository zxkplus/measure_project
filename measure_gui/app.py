...
【棋盘格标定（可选）】
  - 工具 → 棋盘格标定... 打开标定对话框
  - 加载标定板图片（棋盘格可只拍到一部分），输入格子边长 (mm)
  - 在棋盘格区域拉框（点击定中心 → 拖拽调大小 → 滚轮旋转 → 双击确认），只检测框内角点
  - 「检测框内角点」重跑检测；「重新拉框」清除旧框与角点重新画框
  - 误检多时：调高「角点质量」或「最小间距」阈值（自动滤除不属棋盘格的杂点），实时重检
  - 可交互编辑：左键空白=添加角点，左键拖拽=移动，右键=删除（实时重算比例）
  - 「应用标定」后，测量结果（距离/半径/线长）同时显示 mm 和像素值
  - 未标定时结果保持像素单位；「清除标定」可随时回到像素模式
