...
class TemplateView(tk.Frame):
    ...
    def _on_click(self, event):
        ...
        if self._current_tool == TemplateTool.SELECT:
            ...
            if self._select_tool_at(event.x, event.y):
                # Clicked on a tool — selection handled
                return
    ...
    def _select_tool_at(self, cx: float, cy: float) -> bool:
        """Select a tool at canvas position. Returns True if a tool was found."""
        # Deselect all
        for t in self._tools:
            t["_selected"] = False
